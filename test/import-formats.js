  ...
  describe('CommonJS export configuration', () => {
    it('should have correct exports field in package.json', async () => {
      const packageJson = JSON.parse(
        await import('node:fs/promises').then((fs) => fs.readFile(join(__dirname, '..', 'package.json'), 'utf8'))
      );
      ...
      assert.strictEqual(packageJson.exports.import, './lib/index.modern.js', 'import should point to ESM build');

      assert.strictEqual(packageJson.exports.require, './lib/index.cjs', 'require should point to CommonJS build');
    });
  });
});
