...
async function getOptimizer(flag) {
  ...
  try {
    ...
    if (!importPromises.has(flag)) {
      importPromises.set(flag, import(OPTIMIZER_MAP[flag]).then(module => {
...
function optimizeHTML(userOptions = {}) {
  ...
  const options = _extends({}, BASE_OPTIONS, userOptions.aggressive ? DEFAULT_AGGRESSIVE_OPTIONS : {}, userOptions);
   ...
   */
  const plugin = async function plugin(files, metalsmith, done) {
    try {
